...
import com.example.moneymanagement.presentation.view.adapter.JarCategoryAdapter
...
class JarDetailActivity :
    ...
    private lateinit var adapter: JarCategoryAdapter
    ...
    override fun initializeComponent() {
        ...
        adapter = JarCategoryAdapter(emptyList())
        ...
        viewModel.getData(this, budgetEntity.imgBudget)
    ...
    private fun getAllCategory() {
        ...
        viewModel.dataBudget.observe(this) {
            var totalMoney: Int = 0
            adapter.setDataBase(it)
            totalMoney = it.sumOf { totalMoney -> totalMoney.totalMoney }
            val formatTotalMoney = formatMoney(totalMoney)

            if(totalMoney <= 0){
                binding.txtTotalMoney.text = "$formatTotalMoney đ"
            } else {
                binding.txtTotalMoney.text = "- $formatTotalMoney đ"
