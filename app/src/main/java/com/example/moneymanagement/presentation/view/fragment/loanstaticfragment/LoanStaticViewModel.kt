package com.example.moneymanagement.presentation.view.fragment.loanstaticfragment
...
class LoanStaticViewModel : ViewModel() {
    ...
    private fun processData() {
        ...
        val groupType = cachedList.filter { it.type == "loan" }
            .filter { item ->
                try {
        ...
        // Pie chart
        val totalIncome = groupType.sumOf { it.amount }
        val pieEntries = groupType.groupBy { it.nameTypeCategory }
            .map { (nameCategory, items) ->
                val totalMoneyTypeCategory = items.sumOf { it.amount }
                val per = if (totalIncome > 0) (totalMoneyTypeCategory.toDouble() / totalIncome.toDouble()) * 100 else 0.0
                PieEntry(per.toFloat(), nameCategory, items.first().imgTypeCategory)
            }
        ...
        // Category static
        val totalMoneyLoan = groupType.sumOf { it.amount }
        ...
        val parentNameCategory = groupMonth.map { (dateMonth, items) ->
            val childList = items.groupBy { it.nameTypeCategory }
                .map { (categoryName, list) ->
                    ...
                    val processValue = if (totalMoneyLoan > 0) (totalMoney * 100f) / totalMoneyLoan else 0f
